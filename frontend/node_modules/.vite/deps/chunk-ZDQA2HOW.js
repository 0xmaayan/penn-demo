...
var MultiEd25519PublicKey = class _MultiEd25519PublicKey {
  ...
  toBytes() {
    ...
    this.public_keys.forEach((k, i2) => {
      bytes.set(k.value, i2 * Ed25519PublicKey.LENGTH);
    });
  ...
  static deserialize(deserializer) {
    ...
    const keys = [];
    for (let i2 = 0; i2 < bytes.length - 1; i2 += Ed25519PublicKey.LENGTH) {
      const begin = i2;
...
var _MultiEd25519Signature = class _MultiEd25519Signature2 {
  ...
  toBytes() {
    ...
    this.signatures.forEach((k, i2) => {
      bytes.set(k.value, i2 * Ed25519Signature.LENGTH);
    });
  ...
  static deserialize(deserializer) {
    ...
    const sigs = [];
    for (let i2 = 0; i2 < bytes.length - bitmap.length; i2 += Ed25519Signature.LENGTH) {
      const begin = i2;
...
var TransactionBuilderRemoteABI = class {
  ...
  async build(func, ty_tags, args) {
    ...
    const typeArgABIs = abiArgs.map(
      (abiArg, i2) => new ArgumentABI(`var${i2}`, new TypeTagParser(abiArg, ty_tags).parseTypeTag())
    );
    const entryFunctionABI = new EntryFunctionABI(
      ...
      "",
      // Doc string
      funcAbi.generic_type_params.map((_, i2) => new TypeArgumentABI(`${i2}`)),
      typeArgABIs
    );
  ...
  async simulateTransaction(accountOrPubkey, rawTransaction, query) {
    ...
    if (accountOrPubkey instanceof AptosAccount) {
      ...
      const txnBuilder = new TransactionBuilderMultiEd25519(() => {
        ...
        const bits = [];
        const signatures = [];
        for (let i2 = 0; i2 < threshold; i2 += 1) {
          bits.push(i2);
...
// node_modules/.pnpm/@aptos-labs+wallet-adapter-core@3.6.0_@aptos-labs+ts-sdk@1.7.0_aptos@1.21.0/node_modules/@aptos-labs/wallet-adapter-core/dist/index.mjs
...
var WalletCore = class extends import_eventemitter32.default {
  ...
  async signTransaction(transactionOrPayload, asFeePayer, options) {
    ...
    try {
      ...
      const accountAuthenticator = new a(
        new l(publicKey),
        new d(signature)
      );
        ...
        if (bitmap) {
          ...
          if (signature.length < minKeysRequired) {
            ...
            const bits = Array.from(bitmap).flatMap(
              (n) => Array.from({ length: 8 }).map((_, i2) => n >> i2 & 1)
            );
            const index = bits.map((_, i2) => i2).filter((i2) => bits[i2]);
            const publicKeys = this._account.publicKey;
            const matchedPublicKeys = publicKeys.filter(
              (_, i2) => index.includes(i2)
            );
            verified = true;
            for (let i2 = 0; i2 < signature.length; i2++) {
              const isSigVerified = import_tweetnacl3.default.sign.detached.verify(
                ...
                import_buffer.Buffer.from(signature[i2], "hex"),
                import_buffer.Buffer.from(matchedPublicKeys[i2], "hex")
              );
              if (!isSigVerified) {
                verified = false;
                break;
...
buffer/index.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-ZDQA2HOW.js.map
